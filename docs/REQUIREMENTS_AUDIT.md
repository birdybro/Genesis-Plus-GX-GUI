...
| 52. Scope exclusions | Superseded only by explicit follow-up requests | The original release excluded netplay, achievements, cloud, online metadata, TAS, streaming, and expansive debugger tooling. Later user requests explicitly authorized the hidden debugger, netplay, and optional RetroAchievements frontend. Each remains separated from core algorithms and has its own bounded/security gates. Cloud, online metadata/art, signed updates, and TAS/movie/streaming work remain later sequential milestones rather than being inferred into this one. |
