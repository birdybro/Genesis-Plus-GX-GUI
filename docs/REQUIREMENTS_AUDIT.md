...
| 52. Scope exclusions | Superseded only by explicit follow-up requests | The original release excluded netplay, achievements, cloud, online metadata, signed updates, TAS, streaming, and expansive debugger tooling. Later user requests explicitly authorized their sequential implementation. Each remains separated from authoritative core algorithms and has its own bounded, deterministic, privacy, security, lifecycle, and hosted-CI gates; unrelated exclusions remain excluded. |
