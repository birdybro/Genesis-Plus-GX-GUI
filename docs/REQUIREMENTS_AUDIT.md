...
| 52. Scope exclusions | Superseded only by explicit follow-up requests | The original release excluded netplay, achievements, cloud, online metadata, TAS, streaming, and expansive debugger tooling. Later user requests explicitly authorized the hidden debugger, netplay, optional RetroAchievements, opt-in WebDAV cloud frontend, and opt-in licensed online metadata/art. Each remains separated from core algorithms and has its own bounded/security gates. Signed updates and TAS/movie/streaming remain later sequential milestones rather than being inferred into this one. |
