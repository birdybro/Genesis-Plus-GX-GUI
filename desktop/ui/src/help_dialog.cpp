...
QString userGuideText()
{
  return HelpDialog::tr(
    ...
    "\n"
    "Emulation → Emulation Speed provides normal-speed presets and an editor "
    "for exact normal, slow-motion, and fast-forward percentages. Host audio "
    "plays only at 100% speed so alternate pacing cannot create buffer drift.\n"
    "\n"
    "Tools → TAS and Input Movies records deterministic frame-boundary input "
    "or edits a saved timeline. Tools → Local A/V Streaming Output exposes the "
    "documented native frame/audio protocol to loopback clients only.\n"
    "\n"
